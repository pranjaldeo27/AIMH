...
def plot_stigma_by_model(subdf, out_path, title_suffix=""):
    means, cis = [], []
    for model in MODELS:
        scores = []
        for q in QUESTIONS:
            scores.extend(subdf[f"{model}_{q}"].dropna().tolist())
        scores = np.array(scores)
        n = len(scores)
        mean = scores.mean()
        ci95 = stats.sem(scores) * stats.t.ppf(0.975, n - 1) if n > 1 else 0.0
        means.append(mean)
        cis.append(ci95)
    ...
    ax.set_title(f"Overall Stigma Score by Model (95% CI, all questions pooled){title_suffix}",
...
def plot_stigma_by_question_model(subdf, out_path, title_suffix=""):
    group_means = {model: [] for model in MODELS}
    group_cis = {model: [] for model in MODELS}
    for q in QUESTIONS:
        for model in MODELS:
            scores = subdf[f"{model}_{q}"].dropna().to_numpy()
            n = len(scores)
            mean = scores.mean()
            ci95 = stats.sem(scores) * stats.t.ppf(0.975, n - 1) if n > 1 else 0.0
    ...
    ax.set_ylabel("Mean stigma score (1–5)", fontsize=10, color=INK_SECONDARY)
    ax.set_title(f"Stigma Score by Question and Model (95% CI){title_suffix}",
                 fontsize=12, color=INK_PRIMARY, pad=14)
    ...
    fig.tight_layout()
    fig.savefig(out_path, facecolor=SURFACE)
    print(f"Saved grouped bar chart to {out_path}")


# ── 8. Bar: Overall Mean Score by Question, pooled across models (95% CI) ───

def plot_stigma_by_question(subdf, out_path, title_suffix=""):
    q_means, q_cis = [], []
    for q in QUESTIONS:
        scores = np.array([v for model in MODELS for v in subdf[f"{model}_{q}"].dropna().tolist()])
        n = len(scores)
        mean = scores.mean()
        ci95 = stats.sem(scores) * stats.t.ppf(0.975, n - 1) if n > 1 else 0.0
        q_means.append(mean)
    ...
    ax.set_xticklabels(Q_LABELS, fontsize=9.5, color=INK_PRIMARY)
    ax.set_ylabel("Mean stigma score (1–5)", fontsize=10, color=INK_SECONDARY)
    ax.set_title(f"Overall Stigma Score by Question (95% CI, all models pooled){title_suffix}",
                 fontsize=12, color=INK_PRIMARY, pad=14)
    style_axes(ax)
...
# ── 9. Grouped Bar: Overall Stigma Score by Crisis Type x Model (95% CI) ────

def plot_stigma_by_crisis_type_model(full_df, crisis_labels, out_path):
    group_means = {model: [] for model in MODELS}
    group_cis = {model: [] for model in MODELS}
    for ctype in crisis_labels:
        subdf = full_df[full_df[ctype] == 1]
        for model in MODELS:
            scores = []
            for q in QUESTIONS:
                scores.extend(subdf[f"{model}_{q}"].dropna().tolist())
    ...
    ax.set_ylabel("Mean stigma score (1–5)", fontsize=10, color=INK_SECONDARY)
    ax.set_title("Overall Stigma Score by Crisis Type and Model (95% CI, all questions pooled)",
                 fontsize=12, color=INK_PRIMARY, pad=14)
...
# ── Run all three charts once for the full dataset, then once per crisis type ──

CRISIS_GROUPS = [("All", df)] + [
    (ctype, df[df[ctype] == 1]) for ctype in CRISIS_TYPES if df[ctype].sum() > 0
]

for label, subdf in CRISIS_GROUPS:
    suffix = "" if label == "All" else "_" + label.lower().replace("/", "_").replace(" ", "_")
    title_suffix = "" if label == "All" else f" — {label}"

    print("\n" + "=" * 60)
    print(f"CHARTS — {label} (n={len(subdf)})")
    print("=" * 60)
    plot_stigma_by_model(subdf, f"{OUTPUT_DIR}/stigma_by_model{suffix}.png", title_suffix)
    plot_stigma_by_question_model(subdf, f"{OUTPUT_DIR}/stigma_by_question_model{suffix}.png", title_suffix)
    plot_stigma_by_question(subdf, f"{OUTPUT_DIR}/stigma_by_question{suffix}.png", title_suffix)

print("\n" + "=" * 60)
print("CHART — Overall Stigma Score by Crisis Type x Model")
print("=" * 60)
crisis_labels_with_data = [ctype for ctype in CRISIS_TYPES if df[ctype].sum() > 0]
plot_stigma_by_crisis_type_model(df, crisis_labels_with_data, f"{OUTPUT_DIR}/COMBINED/stigma_by_crisis_type_model.png")
